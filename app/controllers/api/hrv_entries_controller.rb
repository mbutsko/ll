class Api::HrvEntriesController < Api::BaseController
  def create
    ...
    entries.each do |entry_params|
      entry = current_user.hrv_entries.find_by(date: entry_params[:date])
      if entry
        entry.update!(value: entry_params[:value])
        updated += 1
      else
        current_user.hrv_entries.create!(date: entry_params[:date], value: entry_params[:value])
        created += 1
      end
    end
